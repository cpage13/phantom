...
|---|---|---|---|
| C1 | Critical | Orphan body GC (Phase 2 § 3.2.1) | `src/phantom-service/tests/unit/test_bulk_delete_body_files.py` (Phase 2 unit). E2E body-lost-then-cleaned coverage falls out of the body-orphan janitor unit tests in `src/phantom-service/tests/unit/test_body_orphan_janitor.py`. |
| C2 | Critical | Saturation try/finally (Phase 2 § 3.2.2 / H1) | `src/phantom-service/tests/unit/test_saturation_release_on_admission_failure.py` (Phase 2). |
| H1 | High | Saturation try/finally (Phase 2 § 3.2.2) | Same as C2 — H1 and C2 collapse into one fix; Phase 2 added the regression test. |
| H2 | High | Content-Length precheck (Phase 2 § 3.2.3) | `src/phantom-service/tests/unit/test_admission_content_length.py` (Phase 2). |
...
| H4 | High | Body-retention contract + recovery carve-out (Phase 2 § 3.2.4) | `src/phantom-service/tests/unit/test_body_retention.py` + `src/phantom-service/tests/unit/test_recovery.py::test_recovery_h4_carveout_discarded_body_not_quarantined` (Phase 2). |
| H6 | High | AdMinter into composition-root TaskGroup (Phase 2 § 3.2.5) | `src/phantom-service/tests/unit/test_composition_taskgroup.py` (Phase 2). The H10(b) test below covers the cancellation contract end-to-end. |
...
| H8 | High | BodyMissingError → corrupted (Phase 2 § 3.2.6) | `src/phantom-service/tests/unit/test_sender_body_missing.py` (Phase 2) + `tests/e2e/test_files_lost_midway.py::test_manual_body_file_lost_post_persist` (Phase 5) + `tests/e2e/regression/test_h10_silent_route_and_supervision.py::test_h10a_missing_body_raises_body_missing_error` (Phase 5). |
