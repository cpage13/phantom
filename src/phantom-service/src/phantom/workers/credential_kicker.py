...
1. the freshness oracle is the **CredentialStore** keyed by host alone
   (``signer_creds.get(row.endpoint)``) instead of the token cache keyed by
   ``(endpoint, uid)``;
    ...
    async def _rescan(self) -> None:
        ...
        for row in rows:
            ...
            try:
                ...
                logger.warning(
                    "CredentialKicker: no route matches endpoint=%s for chain_id=%s; "
            ...
            # list_non_terminal (now in TERMINAL_STATES), never woken.
            # ``release_saturation=False``: this parked ``auth_expired`` row's
            # slot was ALREADY released at park (``_on_auth_failure``), so the
            # writer must NOT re-release it (a double-free that transiently
            ...
            if deadline is not None and (now - row.received_at).total_seconds() > deadline:
                ...
                continue
            cred_row = await signer_creds.get(HostCredKey(row.endpoint))
            ...
            if not isinstance(result, AdmissionGranted):
                ...
                continue
            logger.info(
                "CredentialKicker waking row chain_id=%s for endpoint=%s",
                row.chain_id,
                row.endpoint,
            )
