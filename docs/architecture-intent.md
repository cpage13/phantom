...
|   |   - Sender pool   (N workers, default 4)        |                                |
|   |   - Reaper                                      |                                |
|   |   - AuthKicker                                  |                                |
|   |   - VacuumScheduler                             |                                |
|   |   - AdMinter.run() loop                         |                                |
...
| **Reaper** | `workers/reaper.py` | Deletes terminal-state rows per the retention YAML. Iterates `succeeded`, `failed`, `cancelled`, `stored`, `corrupted`, `auth_expired` on the same sweep. Trims `idempotency_index`. | UPDATE `body_discarded_at`; DELETE terminal rows past retention. | Periodic (`reaper_interval_seconds`, default 60). |
| **AuthKicker** | `workers/auth_kicker.py` | Wakes `auth_expired` rows when a fresh token lands in the cache. Skips body-discarded rows (R6-3); re-admits through the saturation gate, returning the slot on every outcome except a confirmed wake (R9-3 / R10-2). | UPDATE the `auth_expired → queued` wake via the M-W4-F7-guarded `record_attempt_result(expected_state="auth_expired")` - the kicker is the only allowed mover of that transition (doc correction 2026-06-12: this cell previously claimed no `uploads` writes). | `TokenCache.set` fires, plus a 1 s periodic rescan. |
...
| **Recovery sweep** | `workers/recovery.py` | Five-step boot-time sweep (Phase 1 + Phase 2): (1) reset `attempting → queued`; (2) `body_location='file'` rows whose body files vanished → `corrupted` (subject to the invariant #1 carve-out for `body_discarded_at IS NOT NULL`); (3) `body_location='ram'` rows are quarantined (the RAM body store is empty after restart by design); (4) `body_hashes` ↔ `BodyStore.has_body_ref` consistency walk; (5) start the body-orphan janitor for the first sweep. | UPDATE boot-time corrections; transition to `corrupted` for integrity-guard hits. | Lifespan startup, before sender pool starts. |
| **Chain executor** | `chain/executor.py` | NOT a coroutine — called synchronously by the sender per step. One call = one step. Owns capture-TTL gate, substitution, idempotency-header injection, auth injection, send, classify response. Emits DEBUG-level structured logs with `captures` and `sensitive_captures` extras (gated on `isEnabledFor`). | None (sender does the UPDATE). | Sender call. |
...
its own connection and writer, ADR-030). The schema carries
...
The **token cache** lives in the same `uploads.db` and survives
...
Four packages live under `src/`. (A fifth, `phantom-deploy`, exists
for the CDK cloud-side deployment but is out of scope for the
...
- `chain/` — JSONPath wrapper (`jsonpath-ng`), envelope parser
  (raises typed `ParserError` with ADR-010 error codes), **`ChainExecutor`** (the load-bearing primitive). Discriminated-union result types (`Succeeded`, `Failed5xx`, `FailedAuth`, etc.) are exhaustive by mypy.
...
- `routing/` — `resolve_route(url, instance_cfg) -> ResolvedRoute`
  function. No Protocol; one concrete implementation.
- `instances/` — `InstanceContext` dataclass (mutable, so the hot-reload handler can swap `cfg`/`minter`; the docstring is the
  ...
  registry).
- `workers/` — `Sender`, `Reaper`, `AuthKicker`, `VacuumScheduler`,
  `SaturationGate` (returns `AdmissionResult` union), `run_recovery`
  (the four-step commit-aware sweep).
- `observability/` — `logging.py` only. Structlog setup with bearer
  redaction filter and `SensitiveCaptureRedactor` filter. No metrics
  surface, no OTel tracing.
- `routes/` — `POST /v1/send` handler (under 100 lines; the
  acceptance gate is `scripts/check_post_send_size.py`) + the admin
  router + `admission.py` (the `admit_chain` orchestrator, owning
...
- `models/status.py` — `UploadRow`, `UploadState`,
  `TERMINAL_STATES` (the `poll_until` default stop-set: every
  terminal `ChainState` — `succeeded`, `failed`, `cancelled`,
  `stored`, `corrupted`, `auth_expired`; R6-5 added `corrupted`),
...
- `routers/auth.py`, `routers/upstream.py`, `routers/control.py` —
  the three router surfaces. `ReceivedEntry` (the `/control/received`
...
2. **Phantom-the-service is opaque at the auth layer.** Phantom
   never parses tokens. It treats `Authorization` as an opaque byte
   string, stores it in the `(endpoint, uid)`-keyed cache, and
   replays it on retry. `uid` is caller-supplied (`X-Phantom-Uid`
   header) and never inspected.
...
| 3 | `body_hashes` map keys ↔ body-store ref set | Row walk compares `body_hashes` keys against `BodyStore.list_chain_ids` per chain; mismatch bumps the `body_hash_set_mismatch` bucket of `invariant_violation_total`. |
...
|---|---|---|
| 2 | Saturation-bytes basis matches `body_size_bytes` (closes H5) | `saturation_balance` gauge (in-flight declared bytes) for operator visibility; the audit row walk does NOT read it and no drift label exists (C10 doc correction 2026-06-12). Structurally enforced by the single function signature `admit(declared_bytes: int)` / `release(declared_bytes: int)` - no code path can pass an encoded size - plus the invariant #16 atomic-accounting discipline, pinned by the per-path unit releases and the saturation idle-balance e2e (`tests/e2e/test_saturation_idle_balance.py`). |
...
| 7 | No `attempting` row survives a restart | `attempting_row_count` gauge (steady state should equal the sender's in-flight count; spike means a sender died without `record_attempt_result`). Structurally enforced by the recovery sweep resetting every `attempting → queued` before opening ingress. |
...
Aggregate counters/gauges authored in Phase 3 (full list):
...
- `cold_backup_total` / `cold_backup_failed_total` — counters (Phase 4)
...
- **Pi-class hardware** (Raspberry Pi 4 / 5 class — 2–8 GiB RAM,
  16–256 GiB SD card, ARM64) is the primary target. The smart
...
|---|---|---|
| `GET /v1/admin/observability/counters` | `{ "<counter_name>": { "<label>": <int>, ... } }` | All counters from `MetricsRegistry`. Includes `invariant_violation_total`, `orphan_body_count_total`, `record_attempt_result_no_op_total`, `ram_pressure_signal_total`, `db_quarantine_total`, `cold_backup_total`. |
| `GET /v1/admin/observability/gauges` | `GaugesResponse`: `{ "gauges": [ { "name": "...", "description": "...", "values": { "<label>": <float> } }, ... ] }` - a LIST of gauge objects, not a name-to-value map; the empty-string label bucket is the no-label total (C2 doc correction 2026-06-12). | All gauges. Includes `saturation_balance`, `attempting_row_count`, `body_location_distribution`, `persist_controller_queue_depth`, `ram_body_store_bytes`. |
...
| `GET /v1/admin/quarantine` | `{ "quarantines": [{ "path": "...", "name": "...", "kind": "db" \| "body_store", "bytes": <int>, "reason": "corrupted" \| "mode_switch", "iso": "..." }] }` | Inventory of the FLAT timestamped-sibling backup artifacts in each instance data_root: `uploads.corrupted.<iso>.db` / `bodies.quarantine.<iso>/` from a corruption event (Phase 4), and `uploads.mode_switch.<iso>.db` / `bodies.mode_switch.<iso>/` from a back-up-and-run mode switch. The `reason` field discriminates the two; a `mode_switch` entry is restorable via `POST /v1/admin/quarantine/restore`. Operator surface for "what's been backed up; what do I need to look at?" |
...
| Content-Length over `max_buffered_bytes` (Phase 2 H2) | Pre-stream Content-Length check OR mid-stream byte counter. | 413 `body_too_large` with `{ "limit_bytes": ..., "actual_bytes": ... }`. | `tests/e2e/test_e2e_10_parser_error.py`, `src/phantom-service/tests/unit/test_send_route.py`. |
...
- **Per-package unit tests** (`src/<pkg>/tests/unit/`). pytest, fast
  (< 5s for the emulator, longer for phantom). Every module's
  acceptance criteria are testable. mypy `--strict` passes on every
  package.
- **Per-package integration tests** (`src/<pkg>/tests/integration/`).
  pytest. Each package integrates against `phantom-emulator` via
  `httpx.ASGITransport` (in-process) or `start_server` (real-port).
- **Workspace-root contract tests** (`tests/contract/`). ~240 tests,
  ...
  admin-model alignment tests. `ChainAdminDetail` is admin-only and
  NOT covered by the contract test. (The root `tests/integration/`
  carries a handful more cross-package wiring tests alongside.)
- **Workspace-root E2E suite** (`tests/e2e/`). pytest. ~110 test
  files across the top level plus the `crash_recovery/`, `regression/`,
  `stress/`, `all_ram/`, `ingress_abort/`, and `db_contention/`
  subdirs - roughly 230 functions, of which ~210 run in the default
  lane (the `load` / `perf` / `stress` markers gate out the rest) and
  one is a designed `xfail` (the ADR-011 capture-TTL re-execution
  case). Boots real Phantom + real emulator, drives through the
...
- `scripts/check_persist_ordering.py` — every function that touches
  `body_location` and `'file'` has a preceding `fsync` call (the
  persist-handoff ordering invariant; § 5 invariant #6).
...
| File | Trigger | Jobs |
|---|---|---|
| `per_pr.yml` | every PR | (1) `ruff check` + `ruff format --check`; (2) per-package `mypy --strict` via `scripts/precommit/run_mypy_per_package.sh`; (3) per-package unit tests; (4) workspace contract tests; (5) workspace E2E excluding `-m load` + `-m stress`; (6) the falsifiability scripts above. |
| `nightly_stress.yml` | nightly cron | `pytest -m "stress or load"` — multi-GB body, 1k-burst, 10-minute sustained-concurrent. |

**Run cadence.** Unit + integration: continuously during development.
Contract + E2E (non-load): every PR via `per_pr.yml`. Load + stress:
nightly via `nightly_stress.yml`.
