...
Each instance owns TWO SQLite databases (both WAL journal, both under
the `auto_vacuum=NONE` hard rule for flash wear): `uploads.db`
(operator-tunable `synchronous`, default NORMAL) carrying `uploads` and
`idempotency_index`, and `token_cache.db` (`synchronous=FULL`) carrying
the persistent token cache, its own file so token writes never contend
with the hot uploads writer (ADR-030). Body bytes live in RAM, on disk,
or both, selected per deployment by `body_store.mode`:
...
`corrupted` (queryable via `GET /v1/admin/chains/{chain_id}`) — not
lost without trace.
...
- **Terminal rows are never re-quarantined.** A finished row (succeeded,
  failed, stored, cancelled, corrupted) whose body is gone is *expected*
  — bodies are deleted on success and on terminal cleanup. Recovery skips
...
operator choice. A guard at the composition root refuses to boot
`all_ram` over a data directory that still holds body files from a prior
disk-backed mode (booting `all_ram` there would condemn those rows to
`corrupted` and leak the files); migrate via a disk-backed mode or clear
the body-store root first.
...
`last_error="storage_corruption:bodies_missing"` and does not retry. The
worker does **not** crash — a missing body is a row-level terminal
outcome, not a process-level fault.
...
- **Row-count backstop.** An optional `retention.max_rows` cap lets the
  reaper evict the oldest *terminal* rows once the table exceeds the
  limit (default unbounded, preserving time-only retention). In-flight
  and `auth_expired` rows are never evicted by the cap. This is the
  backstop against an `uploads` table growing without bound between
  time-based reaps.
...
all become due at once when an upstream recovers — a Pi fleet reconnecting
after an outage.
...
| Mode-flip guard over persisted rows | [`src/phantom-service/tests/unit/test_a3_mode_flip_with_persisted_rows.py`](../src/phantom-service/tests/unit/test_a3_mode_flip_with_persisted_rows.py) | Booting `all_ram` over a populated disk-backed root fails closed. |
...
remain lighter on direct coverage — and are tracked as future work —
