...
| 422 | `template_unresolved` | A chain step references a captured value (`{{step.name}}`) that no earlier step produces. The same word is ALSO the prefix of a send-time `last_error` token (`template_unresolved:<step>:<site>:<names>`, written by the sender when a placeholder fails to resolve at execution time). The two are different surfaces: this row is an admission-time 422 error CODE on the ingress response, that one is an operator-visible string on a row that reached the terminal `failed` state. | `{ "template": "<string>", "missing_capture": "..." }` | `PhantomValidationError` | Bug in envelope construction; the upstream client fallback propagates. |
