...
Single-file map of the system: what Phantom is, its runtime topology, the seven reliability invariants and which two are actively asserted vs which five are counter/gauge monitored, the core admission → buffer → upload flow, deployment-target constraints, failure modes, and the CI shape. Aim for one read on first contact; come back for specific sections.
